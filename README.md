...
> В этом репозитории используется **синтетический baseline** (базовый
> препроцессинг + CatBoostRanker на почти сырых признаках), как и допускается
> заданием — цель ДЗ отработать Docker, а не саму ML-модель. Как подключить
> свои реальные данные соревнования — см. раздел
> ["Как подставить свою модель"](#как-подставить-свою-модель-из-соревнования).
